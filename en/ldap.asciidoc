...
for automatically synchronizing users from the home directories, and likewise
...
{CMK} utilizes a persistent connection to the LDAP server. With this persistent
...
When connecting to big environments you can have a scenario where the users are located in a specific domain while the groups are hosted in another, to properly query that you need to refeer to a global catalog server and change the connection port to 3268 or 3269 (SSL). 
...
the slower the synchronization will be to process.
...
the user is to utilize as its login ID in {CMK}. The user will subsequently
...
With the [.guihint]#Lower Case User-IDs# options you can convert the synchronized IDs
...
be discarded. Such a scenario can be largely stopped by the LDAP module in {CMK}.
...
can now be saved and tested via the corresponding button at the bottom of the
page.
...
A synchronization can be manually initiated at any time in [.guihint]#Setup > Users#.
In addition, a user will be synchronized if required when they attempt to log in
and have not yet been synchronized.
...
groups the allocation will not work. Therefore be careful to use the correct syntax and
...
sources can also be utilized to enable an allocation. This can make sense if one
...
role, unless they will be synchronized through the
...
For the synchronization of other user information, as a rule only the activation
...
|[.guihint]#Mega menu icons# |icons_per_item |String | `None, entry` | Choose whether the you want to display one icon per topic (`None`) or per entry (`entry`) in the mega menu
|[.guihint]#Navigation bar icons# |nav_hide_icons_title | String | `None, hide` | Choose whether the icons in the navigation menu shall be labeled (`None`) or not (`hide`)
...
|[.guihint]#Sidebar position# |ui_sidebar_position |String |`None, left` | Choose on which side of the screen you want to see the sidebar (`None` = right, `left` = left)
...
|[.guihint]#User interface theme# |ui_theme |String |`facelift, modern-dark` |User interface theme
...
xref:distributed_monitoring#distr_wato[centralized configuration] you can specify whether,
and which LDAP connections should be synchronized from the remote site.
If not otherwise specified, the remote site itself will synchronize all users of
...
from the central to the remote site(s). The synchronization can also be restricted to
...
The setup can be configured in [.guihint]#Setup > General > Distributed Monitoring > Connection Properties#.
